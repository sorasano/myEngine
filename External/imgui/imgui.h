...
namespace ImGui
{
    ...
    IMGUI_API void          SetWindowFontScale(float scale_);                                            // [OBSOLETE] set font scale. Adjust IO.FontGlobalScale if you want to scale all windows. This is an old API! For correct scaling, prefer to reload font + rebuild ImFontAtlas + call style.ScaleAllSizes().
    ...
    IMGUI_API void                  TableSetBgColor(ImGuiTableBgTarget target_, ImU32 color, int column_n = -1);  // change the color of a cell, row, or column. See ImGuiTableBgTarget_ flags for details.
...
struct ImFont
{
    ...
    IMGUI_API const char*       CalcWordWrapPositionA(float scale_, const char* text, const char* text_end, float wrap_width) const;
